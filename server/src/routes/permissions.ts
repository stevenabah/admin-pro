...
// 获取所有权限（树形结构）
router.get('/tree', authMiddleware, async (req: AuthRequest, res) => {
  try {
    const permissions = await prisma.permission.findMany({
      orderBy: [{ sort: 'asc' }, { createdAt: 'asc' }]
    })
...
router.post('/', authMiddleware, async (req: AuthRequest, res) => {
  try {
    const { name, code, type = 'menu', path, icon, component, sort = 0, parentId } = req.body
...
router.put('/:id', authMiddleware, async (req: AuthRequest, res) => {
  try {
    const { id } = req.params
    const { name, code, type, path, icon, component, sort, parentId } = req.body
